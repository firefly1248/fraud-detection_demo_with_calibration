...
class TimeGroupedKFold(BaseCrossValidator):
    ...
    def split(self, X, y=None, groups=None):
        ...
        train_size = quant_size * math.floor(n_time_coh * (1 - self.val_data_share) / quant_size)

        i = 0
        for i in range(self.n_splits - 1):
            train_index = groups[
                groups.isin(all_time_coh[i * quant_size : (i * quant_size + train_size)])
            ].index.values
            test_index = groups[
                groups.isin(
                    all_time_coh[
                        (i * quant_size + train_size) : ((i + 1) * quant_size + train_size)
                    ]
                )
            ].index.values
